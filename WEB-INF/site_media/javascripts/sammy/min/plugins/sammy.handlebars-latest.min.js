...
(function(b) {
    var c = {compilerCache:{},compile:function(d) {
        if (c.compilerCache[d] == null) {
            var f = c.compileFunctionBody(d);
            var e = new Function("context", "fallback", "Handlebars", f);
            c.compilerCache[d] = function(g, h) {
                return e(g, h, c)
            }
        }
        return c.compilerCache[d]
    },compileToString:function(d) {
        var e = c.compileFunctionBody(d);
        return"function(context, fallback) { " + e + "}"
    },compileFunctionBody:function(d) {
        var e = new c.Compiler(d);
        e.compile();
        ...
        var e = function(g) {
            ...
            this.__get__ = function(h) {
                return c.evalExpression(h, this, this.__stack__)
            }
        };
        e.prototype = f;
        return new e(d)
    },pathPatterns:{},parsePath:function(k) {
        if (k == null) {
            return[0,[]]
        } else {
            if (c.pathPatterns[k] != null) {
                return c.pathPatterns[k]
            }
        }
        var h = k.split("/");
        var m = false;
        ...
        for (var g = 0,e = h.length; g < e; g++) {
            switch (h[g]) {
                case"..":
                    if (m) {
                        throw new c.Exception("Cannot jump out of context after moving into a context.")
                    } else {
                        l += 1
                    }
                    break;
        ...
        if (c.isFunction(i)) {
            f = f + i.call(g, d, h);
            if (e != null && c.isFunction(i.not)) {
                f = f + i.not.call(g, d, e)
            }
        } else {
            if (!c.isEmpty(i)) {
                f = f + c.helperMissing.call(d, i, h)
            }
            if (e != null) {
                f = f + c.helperMissing.not.call(d, i, e)
            }
        }
        return f
        ...
        var f = "";
        if (c.isFunction(h)) {
            f = f + c.filterOutput(h.call(g, d), e)
        } else {
            if (!c.isEmpty(h)) {
                f = f + c.filterOutput(h, e)
            }
        }
        return f
    },handleInvertedSection:function(g, e, f) {
        var d = "";
        if (c.isFunction(g) && c.isEmpty(g())) {
            d = d + f(e)
        } else {
            if (c.isEmpty(g)) {
                d = d + f(e)
    ...
    c.Exception = function(d) {
        this.message = d
    };
    c.SafeString = function(d) {
        this.string = d
    };
    c.SafeString.prototype.toString = function() {
        return this.string.toString()
    };
    c.helperMissing = function(f, h) {
        var e = "";
        if (f === true) {
            return h(this)
        } else {
    ...
    c.helperMissing.not = function(d, e) {
        return e(d)
    };
    c.Compiler.prototype = {getChar:function(e) {
        var d = this.peek(e);
        ...
        if (this.text) {
            this.fn = this.fn + 'out = out + "' + c.escapeText(this.text) + '"; ';
        ...
        var d = new c.Compiler(this.string.slice(this.pointer + 1));
        d.compile(function(f) {
            if (f.peek(3) === "{{/") {
                if (f.peek(e.length + 5) === "{{/" + e + "}}") {
                    f.getChar(e.length + 5);
                    return true
                } else {
                    throw new c.Exception("Mismatched block close: expected " + e + ".")
                }
            }
        });
        ...
        this.addText();
        this.mustache = " ";
        while (f = this.getChar()) {
            if (this.mustache && f === "}" && this.peek() === "}") {
                var h = c.trim(this.mustache).split(/\s+/);
                g = h[0];
                i = this.lookupFor(h[1]);
                this.mustache = false;
                this.getChar();
    ...
    var a = a || {};
    a.compile = c.compile;
    a.compileToString = c.compileToString;
    Sammy = Sammy || {};
