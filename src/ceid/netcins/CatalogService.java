...
public class CatalogService extends DHTService implements SocService {
	...
	private String printQueryResults(Catalog cat) {
		...
		if (cat.getURLCatalogEntries() != null
				&& !cat.getURLCatalogEntries().isEmpty()) { // URLContentCatalogEntry
															// PART
			Iterator<URLCatalogEntry> it = cat.getURLCatalogEntries()
					.iterator();
			URLCatalogEntry uce;
			...
			while (it.hasNext()) {
				...
				if (cat instanceof ScoreCatalog)
					buffer.append("Score : "
							+ ((ScoreCatalog) cat).getScores().get(i)
									.toString());
				i++;
			}

		} else if (cat.getContentCatalogEntries() != null
				&& !cat.getContentCatalogEntries().isEmpty()) { // ContentCatalogEntry
																// PART
			Iterator<ContentCatalogEntry> it = cat.getContentCatalogEntries()
					.iterator();
		...
		} else if (cat.getUserCatalogEntries() != null
				&& !cat.getUserCatalogEntries().isEmpty()) { // UserCatalogEntry
																// PART
			Iterator<UserCatalogEntry> it = cat.getUserCatalogEntries()
					.iterator();
