...
public class ClientMain {
	...
	/**
	 * timeSource Here all the pre- issues must be covered.
	 */
			...
			try {
				...
				case SEARCHCONTENT:
					...
					if (lastarg.startsWith("-s")) {
						...
						this.driver.execRequests.add(new SearchContentRequest(
								sargs[2], sourceNum));
					} else {
						// Add the Request for search
						this.driver.execRequests.add(new SearchContentRequest(
			...
			if (args[i].equals("help")) {
				if (i + 1 < args.length) {
					...
					System.out
							.println("*                                                                          *");
					System.out
							.println("*  2.searchuser [-s <source number>] <query>                               *");
					System.out
							.println("*                                                                          *");
					System.out
					...
					System.out
							.println("*    |(show pending|friends|shared <node number>)                          *");
					System.out
							.println("*                                                                          *");
					System.out
							.println("*  6.exit                                                                  *");
					...
					System.out
							.println("*                                                                          *");
					System.out
							.println("*  9.searchcontent [-s <source number>] <query>                            *");
					System.out
							.println("*                                                                          *");
					System.out
	...
	private void executeScenario(Vector<org.w3c.dom.Node> index,
		...
		for (org.w3c.dom.Node tmp : index) {
			...
			if (((Element) currentNode).getTagName().equals("source")) {
				...
				if (((Element) currentNode).getTagName().equals("directory")) {
					...
					if (sfile.exists() && sfile.isFile()) {
						System.out.println("File " + sf
								+ " is being indexed now!");
						this.driver.execRequests.add(new IndexContentRequest(
								sfile.getAbsolutePath(), sourceNum));
						doNotify();
					} else
					...
					if (!profile.isEmpty()) {
						...
						this.driver.execRequests.add(new IndexUserRequest(
								profile, sourceNum));
						doNotify();
					} else
				...
				if (sfile.exists() && sfile.isFile()) {
					...
					this.driver.execRequests.add(new IndexContentRequest(sfile
							.getAbsolutePath()));
					doNotify();
				} else
					System.out.println("Single File is not valid!");
				...
				// user_address
				currentNode = children.item(0);
				String udata = ((Text) currentNode.getFirstChild()).getData()
						.trim();
				profile.put("user address", udata);
		...
		for (org.w3c.dom.Node tmp : search) {
			...
			if (((Element) currentNode).getTagName().equals("source")) {
				...
				// int k =
				// Integer.parseInt(((Text)currentNode.getFirstChild()).getData().trim());

				// Add the Request for search
				this.driver.execRequests.add(new SearchContentRequest(
						queryTerms, sourceNum)); // Feed the whole query!
				doNotify();
				...
				// Add the Request for search
				this.driver.execRequests.add(new SearchContentRequest(
						queryTerms)); // Feed the whole query!
				doNotify();
			}
		}
	...
	private void directoryIndexing(File f, int sourceNum) {
		...
		while (!dir.isEmpty()) {
			...
			if (innerf.isDirectory()) {
				...
				// Add the Request for indexing the file
				this.driver.execRequests.add(new IndexContentRequest(innerf
