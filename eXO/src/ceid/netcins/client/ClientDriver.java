 ...
 * social network services. It contains the methods which resolve the high level
 * user requests. Theser operations are handled by the REQUESTDISPATCHER Thread.
 * 
 * @author andy
 */
public class ClientDriver {
  ...
  private void init() throws Exception{
      ...
      if(localAddress.isLoopbackAddress()){
          ...
          Socket temp = new Socket(environment.getParameters().getString("pastry_socket_known_network_address"), 
                environment.getParameters().getInt("pastry_socket_known_network_address_port"));
          if (temp.getLocalAddress().equals(localAddress)) throw new IllegalStateException("No connection to the Internet: "+localAddress);
            localAddress = temp.getLocalAddress();
            temp.close();
      }
      
      // Generate the NodeIds Randomly RandomNodeIdFactory, IPNodeIdFactory (alternative)
      NodeIdFactory nidFactory = new IPNodeIdFactory(localAddress, BINDPORT, environment);
      ...
      Storage stor = new PersistentStorage(idf,"SoNATA_MetadataFile_"+this.BINDPORT,".",-1,node.getEnvironment());
      storage = new StorageManagerImpl(idf,stor,new LRUCache(new MemoryStorage(idf),512*1024,node.getEnvironment()));
      
      // Queue for Requests
      execRequests=new Vector<Request>();
      
      // TOO : Fix the User UIDs registration!!!!
      past = new CatalogService(node, storage, REPLICATION_FACTOR, INSTANCE,new User(node.getNodeId()));
...
private void runReplicaMaintence() {
  ...
  /**
   * Terminates the contstructor Thread
...
protected void serveRequest(Request req) throws Exception{
      ...
      if(req instanceof IndexContentRequest){
          ...
          if(ireq.getFilePath() != null){
              ...
              past.indexContent(new File(ireq.getFilePath()), 
                    ...
                    public void receiveResult(Object result) {
                        ...
                        System.out.println("File : "+ireq.getFilePath()+", indexed successfully");
                        ...
                        if(result instanceof Boolean[]){
                            ...
                            if(ireq.t!=null){
                                ...
                                try {
                                    ...
                                    java.util.logging.Logger.getLogger(ClientDriver.class.getName()).log(Level.SEVERE, null, ex);
                                }
                            }
                    ...
                    public void receiveException(Exception result) {
                        System.out.println("File : "+ireq.getFilePath()+", indexed with errors : "+result.getMessage());
                    }   
              });
          }
          ...
          if(ireq.getUserProfileMap() != null){
              ...
              past.indexUser( 
                    ...
                    public void receiveResult(Object result) {
                        ...
                        System.out.println("User : "+past.getUser().getUID()+", indexed successfully");
                        // TODO : Check the replicas if are updated correctly!
                        // run replica maintenance
                        //runReplicaMaintence();
                        if(result instanceof Boolean[]){
                    ...
                    public void receiveException(Exception result) {
                        System.out.println("User : "+past.getUser().getUID()+", indexed with errors : "+result.getMessage());
                    }   
              });
          }
          ...
          if(query!=null){  // Handle the query appropriately
              ...
              past.searchQuery(QueryPDU.CONTENT_ENHANCEDQUERY, 
                      query,
                      screq.getK(),
                      new Continuation(){

                    public void receiveResult(Object result) {
                        System.out.println("Content Query : "+screq.getQuery()+", Top Results returned successfully");
                        // debugging only
...
//                            System.out.println("Result : "+result);
                         ...
                         // Debugging only!!!
                        if(screq.t!=null){
                            String msg = "Content Query : "+screq.getQuery()+", Top Results returned successfully";
                            try {
                    ...
                    public void receiveException(Exception result) {
                        System.out.println("Content Query : "+screq.getQuery()+", result (error) "+result.getMessage());
                    }
          ...
          if(query!=null){  // Handle the query appropriately
              ...
              past.searchQuery(QueryPDU.USERQUERY, 
                      query,
                      sureq.getK(),
                      new Continuation(){

                    public void receiveResult(Object result) {
                        System.out.println("User Query : "+sureq.getQuery()+", Top Results returned successfully");
                        // debugging only
//                        if(result instanceof Catalog){
//                            System.out.println("Catalog : "+(Catalog)result);
//                        }else
//                            System.out.println("Result : "+result);
                    ...
                    public void receiveException(Exception result) {
                        System.out.println("User Query : "+sureq.getQuery()+", result (error) "+result.getMessage());
                    }
          ...
          // TODO : Implement this just like in Simulation but find a way to compute destUID!
          ...
          if(type == StatsRequest.PENDING){
              ...
              Iterator<FriendRequest> it = past.getUser().getPendingFReq().iterator();
              ...
              int i=0;
              while(it.hasNext()){
                  System.out.println("\n - "+(++i)+". File : "+sc.get(it.next()).toString());
              }
              System.out.println("\n*******************************************\n");
