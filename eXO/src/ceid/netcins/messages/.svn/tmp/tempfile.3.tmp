...
/**
 * This message type is sent to indicate that a friend request has been
 * approved by the end user and now the source user must be informed.
 *
 * 
 * @author andy
 */
public class FriendApprMessage extends ContinuationMessage{
  ...
  public static FriendApprMessage build(InputBuffer buf, Endpoint endpoint, PastContentDeserializer pcd) throws IOException {
  ...
  private FriendApprMessage(InputBuffer buf, Endpoint endpoint, PastContentDeserializer pcd) throws IOException {
    super(buf, endpoint);
    if (serType == S_SUB) {
