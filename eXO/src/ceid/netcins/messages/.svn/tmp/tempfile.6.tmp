 ...
 * Specifically, this class will not have to carry any extra data
 * than LookupMessage.
 * 
 * @author andy
 */
public class FriendReqMessage extends ContinuationMessage{
   ...
   */
  public FriendReqMessage(int uid, Id id, NodeHandle source, Id dest, FriendReqPDU frPDU) {
  ...
  public static FriendReqMessage build(InputBuffer buf, Endpoint endpoint, PastContentDeserializer pcd) throws IOException {
  ...
  private FriendReqMessage(InputBuffer buf, Endpoint endpoint, PastContentDeserializer pcd) throws IOException {
    super(buf, endpoint);
    if (serType == S_SUB) {
