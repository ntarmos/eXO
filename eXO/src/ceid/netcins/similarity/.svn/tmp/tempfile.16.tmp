 ...
 * which is waiting until a scoring request has been issued 
 * to the scoringRequest queue! The thread itself is created in 
 * the CatalogService class!
 * 
 * TODO : REFINEMENT 
 * 
 * @author andy
...
public class Scorer {
  ...
  private void serveRequest(SimilarityRequest req) {
      ...
      if((req.getType() == QueryPDU.CONTENTQUERY || req.getType() == QueryPDU.CONTENT_ENHANCEDQUERY) 
          ...
          Iterator<ContentCatalogEntry> it = catalog.getContentCatalogEntries().iterator();
          ...
          // HashMap which contains the CatalogEntries with the corresponding score
          ...
          if(req.getType()== QueryPDU.CONTENT_ENHANCEDQUERY ||req.getType()== QueryPDU.USER_ENHANCEDQUERY 
              ...
              if(cprof !=null ){
                  ...
                  BinaryWeight[] profileWeights1 = new BinaryWeight[docTerms.size()];
                  z = 0;
                  while(it3.hasNext()){
                  ...
                  cossimUserProfiles = new CosineSimilarity(null, profileWeights1);
              }
          }

          // For every entry of Catalog compute 1)global term set and 2) the CosineSimilarity
          while(it.hasNext()){
              // Important to clear in order to be reused!
              docTerms.clear();
              entry = it.next();
              ...
              if(cossimUserProfiles !=null){
                  ...
                  if(cprof != null){
                      ...
                      BinaryWeight[] profileWeights2 = new BinaryWeight[docTerms.size()];
                      z = 0;
                      while(it4.hasNext()){
                      ...
                      scoreBoard.put(entry, new Float( 0.5 * cossim.getScore() + 0.5 * cossimUserProfiles.getScore() ));
                  }else
                      scoreBoard.put(entry, new Float( 0.5 * cossim.getScore() + 0.5 * 0 ));
              }else{
          ...
          LinkedHashMap<ContentCatalogEntry,Float> sortedScoreBoard = this.sortHashMapByValuesD(scoreBoard);
          ...
          ScoreCatalog topK = new ScoreCatalog(req.getCatalog().getTID(),v1,v2);
          req.getContinuation().receiveResult(new ResponsePDU(req.getMessagesCounter(), topK));
      ...
      // **************  USER SEARCHING PART  **************
          ...
          // Compute Global Set of terms and feed in the CosineSimilarity
          Iterator<UserCatalogEntry> it = catalog.getUserCatalogEntries().iterator();
          ...
          while(it.hasNext()){
              ...
              // *** User Profile of this entry - global term computation ***
              // Important to clear in order to be reused!
              // NO NEED TO CLEAR THE docTerms because we want the same weights!!!
              if(cossimUserProfiles !=null){
                  // Create the Weights!
                  // 4. ENTRY's USER PROFILE WEIGHTS (ENHANCED QUERY)
                  Iterator<String> it4 = docTerms.iterator();
          ...
          // Sort by float score value
          LinkedHashMap<UserCatalogEntry,Float> sortedScoreBoard = this.sortHashMapByValuesU(scoreBoard);
          ...
          req.getContinuation().receiveResult(new ResponsePDU(req.getMessagesCounter(),catalog));
      }
  }
  ...
  /**
   * This function will return the scored CatalogEntries sorted by the
   * value of score!!!
   * 
   * @param passedMap
   * @return
